...
###   trustnetworkglobalcoincore.org and trustnetworkglobalcoin.org and compares them.
...
WORKINGDIR="/tmp/trustnetworkglobalcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://trustnetworkglobalcoincore.org"
HOST2="https://trustnetworkglobalcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="trustnetworkglobalcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'trustnetworkglobalcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "trustnetworkglobalcoin.org failed to provide signature file, but trustnetworkglobalcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "trustnetworkglobalcoin.org and trustnetworkglobalcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TrustNetworkGlobalCoin Core binary release signing key installed?"
   fi
