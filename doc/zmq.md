...
The TrustNetworkGlobalCoin Core daemon can be configured to act as a trusted "border
router", implementing the trustnetworkglobalcoin wire protocol and relay, making
...
The ZeroMQ feature in TrustNetworkGlobalCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building trustnetworkglobalcoind:
...
For instance:
    ...
    $ trustnetworkglobalcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/trustnetworkglobalcoind.tx.raw \
...
These options can also be provided in trustnetworkglobalcoin.conf.
...
From the perspective of trustnetworkglobalcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into trustnetworkglobalcoind directly. Furthermore, no information is
...
using. TrustNetworkGlobalCoind appends an up-counting sequence number to each
