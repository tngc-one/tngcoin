...
The TNGC Core daemon can be configured to act as a trusted "border
router", implementing the tngc wire protocol and relay, making
...
The ZeroMQ feature in TNGC Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building tngcd:
...
For instance:
    ...
    $ tngcd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/tngcd.tx.raw \
...
These options can also be provided in tngc.conf.
...
From the perspective of tngcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tngcd directly. Furthermore, no information is
...
using. TNGCd appends an up-counting sequence number to each
