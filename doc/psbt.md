# PSBT Howto for TrustNetworkGlobalCoin Core

Since TrustNetworkGlobalCoin Core 0.17, an RPC interface exists for Partially Signed TrustNetworkGlobalCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/trustnetworkglobalcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for TrustNetworkGlobalCoin transactions that are not fully signed
...
## PSBT in TrustNetworkGlobalCoin Core
...
#### Multisig with multiple TrustNetworkGlobalCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
TrustNetworkGlobalCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `trustnetworkglobalcoin-cli` is used.
...
Later, when *V* TNGC has been received on *Amulti*, and Bob and Carol want to
