TrustNetworkGlobalCoin Core version 0.11.0 is now available from:

  <https://trustnetworkglobalcoin.org/bin/trustnetworkglobalcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer TNGC/kB fee to be
...
For example, add the following to `trustnetworkglobalcoin.conf`:
...
related to the blockchain in the trustnetworkglobalcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows TrustNetworkGlobalCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that TrustNetworkGlobalCoin Core will 
...
For an example script see [trustnetworkglobalcoin-submittx](https://github.com/laanwj/trustnetworkglobalcoin-submittx).
...
- #5334 `cf87536` libtrustnetworkglobalcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new trustnetworkglobalcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only TrustNetworkGlobalCoinAmountField
- #5753 `0f44672` Add trustnetworkglobalcoin logo to about screen
...
- #5809 `46bfbe7` Add trustnetworkglobalcoin-cli man page
...
- #5849 `41259ca` contrib/init/trustnetworkglobalcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename TrustNetworkGlobalCoin-Qt.app to TrustNetworkGlobalCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for trustnetworkglobalcoin-tx)
...
- Ben Holden-Crowther
- bikinibabe
- TrustNetworkGlobalCoinPRReadingGroup
- Blake Jakopovic
- BtcDrak
- charlescharles
- Chris Arnesen
