TrustNetworkGlobalCoin Core version 0.18.0 is now available from:

  <https://trustnetworkglobalcoincore.org/bin/trustnetworkglobalcoin-core-0.18.0/>
...
`/Applications/TrustNetworkGlobalCoin-Qt` (on Mac) or `trustnetworkglobalcoind`/`trustnetworkglobalcoin-qt` (on
Linux).
...
TrustNetworkGlobalCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use TrustNetworkGlobalCoin Core on unsupported systems.
...
10.10. Additionally, TrustNetworkGlobalCoin Core does not yet change appearance when
...
`contrib/init/trustnetworkglobalcoind.service`, it has been changed to use
`/var/lib/trustnetworkglobalcoind` as the data directory instead of
`~trustnetworkglobalcoin/.trustnetworkglobalcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/trustnetworkglobalcoind` will
exist has enough space (check using `df -h /var/lib/trustnetworkglobalcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://trustnetworkglobalcoincore.org/en/contact/) the TrustNetworkGlobalCoin Core
...
The systemd init file (`contrib/init/trustnetworkglobalcoind.service`) has been changed
to use `/var/lib/trustnetworkglobalcoind` as the data directory instead of
`~trustnetworkglobalcoin/.trustnetworkglobalcoin`. This change makes TrustNetworkGlobalCoin Core more consistent with
...
`datadir` option in `/etc/trustnetworkglobalcoin/trustnetworkglobalcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/trustnetworkglobalcoin/trustnetworkglobalcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/trustnetworkglobalcoin-conf.md)
  about the `trustnetworkglobalcoin.conf` file describes how to use it to configure
  TrustNetworkGlobalCoin Core.

- A new document introduces TrustNetworkGlobalCoin Core's BIP174 [Partially-Signed
  TrustNetworkGlobalCoin Transactions
  (PSBT)](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent TrustNetworkGlobalCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  TrustNetworkGlobalCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when TrustNetworkGlobalCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of TrustNetworkGlobalCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/trustnetworkglobalcoin/trustnetworkglobalcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart trustnetworkglobalcoind with the `-deprecatedrpc=generate`
...
- A new `trustnetworkglobalcoin-wallet` tool is now distributed alongside TrustNetworkGlobalCoin
...
This section describes planned changes to TrustNetworkGlobalCoin Core that may affect
other TrustNetworkGlobalCoin software and services.

- Since version 0.16.0, TrustNetworkGlobalCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with TrustNetworkGlobalCoin Core 0.20 (expected about
  a year after 0.18), TrustNetworkGlobalCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the TrustNetworkGlobalCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in TrustNetworkGlobalCoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any TrustNetworkGlobalCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, TrustNetworkGlobalCoin Core
...
- A sub-project of TrustNetworkGlobalCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with TrustNetworkGlobalCoin Core.  See their [project
  page](https://github.com/trustnetworkglobalcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to TrustNetworkGlobalCoin Core's own implementation, although entropy
  gathered by TrustNetworkGlobalCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves TrustNetworkGlobalCoin Core a
...
- On macOS, TrustNetworkGlobalCoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `trustnetworkglobalcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libtrustnetworkglobalcoin_server.a` (jnewbery)
...
- #13926 [Tools] trustnetworkglobalcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like trustnetworkglobalcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `trustnetworkglobalcoin-qt -version` output formatting (hebasto)
...
- #11625 Add TrustNetworkGlobalCoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libtrustnetworkglobalcoin_server` (laanwj)
...
- #15154 Configure: trustnetworkglobalcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/trustnetworkglobalcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for TrustNetworkGlobalCoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libtrustnetworkglobalcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in trustnetworkglobalcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into TrustNetworkGlobalCoin Core (Christewart)
...
- #12246 Bugfix: Only run trustnetworkglobalcoin-tx tests when trustnetworkglobalcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_trustnetworkglobalcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_trustnetworkglobalcoin` (MarcoFalke)
...
- #14186 trustnetworkglobalcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_trustnetworkglobalcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update trustnetworkglobalcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/trustnetworkglobalcoin-conf.md (hebasto)
...
- #15354 Add missing `trustnetworkglobalcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct trustnetworkglobalcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support trustnetworkglobalcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/trustnetworkglobalcoin/).
