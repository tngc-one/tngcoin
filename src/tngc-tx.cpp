...
static void SetupTNGCTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " tngc-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  tngc-tx [options] <hex-tx> [commands]  Update hex-encoded tngc transaction\n" +
            "or:     tngc-tx [options] -create [commands]   Create hex-encoded tngc transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tngc transaction
